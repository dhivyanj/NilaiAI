...
import { getOverview, getPatterns } from './services/api';
...
const Dashboard = ({ onNavigateHome, isDark, setIsDark }) => {
  ...
  const userId = "demo"; // TODO: Replace with auth user ID
    ...
    const fetchData = async () => {
      try {
        ...
        const [overviewData, patternsData] = await Promise.all([
          getOverview(userId, period),
          getPatterns(userId, period)
        ]);
    ...
    <div className={`min-h-screen font-roboto overflow-hidden relative transition-colors duration-300 ${
      ...
      <main className="relative z-10 px-6 py-8">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          {/* LOADING STATE */}
          {loading && (
            <div className={`flex items-center justify-center py-24 rounded-3xl border backdrop-blur-md ${
              isDark
              ...
              <div className="text-center space-y-4">
                ...
                <p className={isDark ? 'text-gray-300' : 'text-gray-700'}>Loading insights...</p>
              </div>
            </div>
          )}
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-3">
                    <span className="text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#FF43D3] to-blue-500">
                      {overview?.score || mockAnalytics.overallRisk}
                    </span>
                    <span className={`text-2xl ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>/100</span>
                  </div>
                  <p className={`text-sm mt-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Status: <span className="text-green-400 font-medium">{overview?.status || mockAnalytics.riskTrend}</span></p>
                </div>
                <div className="text-right">
                  <p className="text-gray-500 text-sm mb-2">Last analyzed: {overview?.lastUpdated || mockAnalytics.lastUpdated}</p>
                  <div className="flex items-center gap-2 justify-end">
                    <div className="flex gap-1">
                      {Array(Math.round((overview?.confidence || mockAnalytics.confidence) / 20)).fill(0).map((_, i) => (
                        <div key={i} className="w-2 h-2 rounded-full bg-[#FF43D3]"></div>
                      ))}
                    </div>
                    <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{Math.round((overview?.confidence || mockAnalytics.confidence) * 100)}% confidence</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(patterns?.patterns || mockAnalytics.patterns).map((pattern, idx) => (
                <div 
                  key={idx}
